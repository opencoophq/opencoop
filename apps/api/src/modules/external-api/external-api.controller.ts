import { Controller, Post, Patch, Body, Req, UseGuards, Get, Query } from '@nestjs/common';
...
export class ExternalApiController {
  ...
  @Get('shareholders/search')
  @ApiOperation({ summary: 'Search shareholders by name (case-insensitive)' })
  async searchByName(@Req() req: any, @Query() dto: SearchByNameDto) {
    const coopId = req.coop.id;
    const results = await this.externalApiService.searchByName(coopId, dto.name);
    return { results };
  }
