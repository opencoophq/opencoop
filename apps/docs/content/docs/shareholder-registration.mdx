...
1. A new **shareholder record** is created
2. A new **purchase transaction** is created with a unique OGM code (status: awaiting payment)
3. The shareholder receives payment details (OGM code + EPC QR code) immediately
4. When the shareholder makes the bank transfer, you can import the bank statement via **Bank Import** — OpenCoop automatically matches the payment to the transaction using the OGM code
5. Once payment is confirmed, the shares are activated
