...
async function main() {
  ...
  // Link both admin users to the coop with the Admin role attached via
  // the coop_admin_roles join table (the new source of truth for role
  // membership). Legacy `roleId` is left null — auth code only reads from
  // the join table.
