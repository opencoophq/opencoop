 ...
 *
 * Creates a demo coop ("Zonnecoöperatie Vlaanderen", slug: demo) with sample data:
 * - 3 demo users: coop admin (admin@zonnecooperatie.be) + 2 shareholders
 * - 6 shareholders: 2 individuals w/ accounts, 1 company, 1 w/o account, 1 pending, 1 minor
 * - 2 projects (solar + wind), 2 share classes (A @ €250, B @ €500)
 * - 8 registrations with payments, 1 dividend period with 5 payouts
 *
 * Safe to run against production — does NOT touch the existing system admin user.
 * Idempotent: uses upserts and existence checks, safe to re-run.
 *
 * Demo login credentials (password: demo1234):
 *   - Coop admin: admin@zonnecooperatie.be
 *   - Shareholder: jan.peeters@email.be
 *   - Shareholder: els.devos@email.be
 *
 ...
 *   3. Run the seed:
 *      DATABASE_URL="postgresql://opencoop:<password>@localhost:5433/opencoop" npx tsx packages/database/prisma/seed-demo.ts
 *   4. Kill the tunnel:
...
function generateOgmCode(prefix, sequence) {
...
async function main() {
    console.log('Seeding demo coop for production...\n');
    const demoPasswordHash = await bcrypt.hash('demo1234', 12);
    // ---------------------------------------------------------------------------
    // 1. USERS — look up existing system admin, create demo users only
    // ---------------------------------------------------------------------------
    const adminUser = await prisma.user.findUnique({
        where: { email: 'wouter.hermans@opencoop.be' },
    });
    if (!adminUser) {
        console.error('  ERROR: System admin user wouter.hermans@opencoop.be not found!');
        process.exit(1);
    }
    console.log(`  Found system admin: ${adminUser.email}`);
    ...
    if (existingDividend) {
        ...
        const regsByShareholder = new Map();
        ...
        for (const shareholder of activeShareholders) {
            ...
            if (regs.length === 0)
                continue;
            let grossAmount = new client_1.Prisma.Decimal(0);
            const breakdown = [];
            for (const reg of regs) {
                ...
                grossAmount = grossAmount.add(new client_1.Prisma.Decimal(gross));
                breakdown.push({
            ...
            const withholdingTax = grossAmount.mul(new client_1.Prisma.Decimal(withholdingTaxRate));
