...
async function main() {
  ...
  for (const tx of oldTransactions) {
    const newType = mapType(tx.type);
    const newStatus = mapStatus(tx.status);

    // For TRANSFER_IN, the share belongs to the recipient
    // For regular BUY/SELL, use the share's class/project
    const shareClassId = tx.shareClassId;
    if (!shareClassId) {
      console.warn(`  WARN: Transaction ${tx.id} has no shareClassId — skipping`);
      continue;
    }
    ...
    const registration = await prisma.registration.create({
      data: {
        ...
        registerDate: tx.purchaseDate || tx.createdAt,
        ogmCode: tx.ogmCode || null,
        certificateNumber: tx.certificateNumber || null,
        fromShareholderId: tx.type === 'TRANSFER_IN' ? tx.fromShareholderId : null,
        toShareholderId: tx.type === 'TRANSFER_OUT' ? tx.toShareholderId : null,
        channelId: tx.channelId,
        processedByUserId: tx.processedByUserId,
    ...
    txToRegMap.set(tx.id, registration.id);
    regCount++;

    // Create payment if the old payment exists and was confirmed (or for completed transactions)
    if (tx.paymentId && tx.paymentAmount) {
      const paymentAmount = parseFloat(tx.paymentAmount);
      const isPaymentConfirmed = tx.paymentStatus === 'CONFIRMED' || tx.paymentStatus === 'MATCHED';
      ...
      if (isPaymentConfirmed || newStatus === 'COMPLETED') {
        await prisma.payment.create({
          data: {
            ...
            bankDate: tx.paymentDate || tx.createdAt,
            bankTransactionId: tx.bankTransactionId || null,
            matchedAt: tx.paymentDate || tx.createdAt,
            createdAt: tx.createdAt,
          },
        });
        payCount++;
